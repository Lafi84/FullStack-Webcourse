...
const Menu = () => {
	...
	return (
		<div>
			...
			<Link style={padding} to="/new_anecdote">create new</Link>
...
const Anecdote = ({ anecdote }) => (
	<div>
		...
		<div>For more info please go to <a href={anecdote.url}>{anecdote.url}</a></div>
	</div>
);
...
const AnecdoteList = ({ anecdotes }) => (
	<div>
		<h2>Anecdotes</h2>
		<ul>
			{anecdotes.map(anecdote => <Link key={anecdote.id} to={'/anecdote/' + anecdote.id}>
...
const Footer = () => (
	<div>
		...
		See <a
			href='https://github.com/fullstack-hy2019/routed-anecdotes/blob/master/src/App.js'>https://github.com/fullstack-hy2019/routed-anecdotes/blob/master/src/App.js</a> for
		the source code.
	</div>
);
	...
	return (
		<div>
			...
			<Router>
				...
				<Route path="/new_anecdote" render={() => <CreateNew addNew={addNew} showNotification={showNotification}/>}/>
				<Route path="/about" render={() => <About/>}/>
				<Route path="/anecdote/:id" render={({ match }) => <Anecdote anecdote={anecdoteById(match.params.id)}/>}/>
			</Router>
			<Footer/>
		</div>
	);
